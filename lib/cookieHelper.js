...
class CookieHelper {
  ...
  setUserLoginCookie(requestObject, responseObject, cookieOptions) {
    ...
    if (!basicHelper.isDevelopment()) {
      setCookieDefaultOptions.secure = true;
    }

    console.log('cookieValue in setUserLoginCookie: ', cookieValue);
    console.log('cookieName in setUserLoginCookie: ', cookieName);
  ...
  deleteUserLoginCookie(requestObject, responseObject, options = {}) {
    if (!basicHelper.isProduction()) {
      deleteCookieOptions.sameSite = 'none';
    }

    if (!basicHelper.isDevelopment()) {
      deleteCookieOptions.secure = true;
    }
  ...
  async validateUserLoginCookieIfPresent(req, res, next) {
    ...
    console.log('Inside validateUserLoginCookieIfPresent: ', userLoginCookieName, loginCookieValue);
    ...
    if (!CommonValidators.isVarNullOrUndefined(loginCookieValue)) {
      const authResponse = await new UserLoginCookieAuth(loginCookieValue, {
        ...
        .perform()
        .catch(function(err) {
          console.log('Inside validateUserLoginCookieIfPresent: err: ', err);
          return err;
        });
      ...
      if (authResponse.isFailure()) {
        ...
        req.internalDecodedParams.current_user = authResponse.data.current_user;
        console.log(
          'Inside validateUserLoginCookieIfPresent: req.internalDecodedParams.current_user: ',
  ...
  async validateUserLoginCookieRequired(req, res, next) {
    ...
    console.log('Inside validateUserLoginCookieRequired: currentUser: ', currentUser);
    console.log('Inside validateUserLoginCookieRequired: req.internalDecodedParams: ', req.internalDecodedParams);
    console.log('Inside validateUserLoginCookieRequired: userLoginCookieName ', userLoginCookieName);
